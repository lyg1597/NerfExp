...
if __name__ == "__main__":
    ...
    data_file = os.path.join(script_dir, f'./data_pc/data_09-16-20-58.pickle')
    ...
    for i in range(100):
        print(i)
        while True:
            X0 = AutoLand.sample_X0()
            center = AutoLand.get_init_center(X0)
            x, e = center 
            success, res, cv_img = perception.set_percept_validate(x, e, fn = None)

            if success:
                # cv2.imshow('Image', cv_img)
                # cv2.waitKey(0)
                # cv2.imwrite(f'res_{i:03d}.png', cv_img)
                data.append((X0, res.reshape((-1,1)), center))
                break
            else:
                print("unsuccess sample")
    with open(data_file, 'wb+') as f:
        pickle.dump(data, f)
