...
data = np.load(os.path.join(script_dir,"tiny_nerf_data_new.npz"))
images = data["images"]
poses = data["poses"]
focal = data["focal"]
envs = data['env']
...
images = torch.Tensor(data["images"][:n_training]).to(device)
poses = torch.Tensor(data["poses"]).to(device)
...
testimg = torch.Tensor(data["images"][testimg_idx]).to(device)
testpose = torch.Tensor(data["poses"][testimg_idx]).to(device)
testenv = torch.Tensor(data["env"][testimg_idx]).to(device)
...
n_restarts = 5  # Number of times to restart if training stalls
...
def train():
    ...
    for i in trange(n_iters):
        ...
        # torch.nn.utils.clip_grad_norm_(fine_model.parameters(), max_norm=1.0) if fine_model else None

        if i%100==0:
            torch.save(model.state_dict(), "nerf.pt")
            torch.save(fine_model.state_dict(), "nerf-fine.pt")
            print("\n", target_img_idx, loss.item())

        for name, param in model.named_parameters():
            if param.grad is not None:
                if torch.isnan(param.grad).any():
                    print(f"NaN detected in model gradient of {name}")
                    return False, train_psnrs, val_psnrs
                if torch.isinf(param.grad).any():
                    print(f"Inf detected in model gradient of {name}")
                    return False, train_psnrs, val_psnrs

        for name, param in fine_model.named_parameters():
            if param.grad is not None:
                if torch.isnan(param.grad).any():
                    print(f"NaN detected in fine model gradient of {name}")
                    return False, train_psnrs, val_psnrs
                if torch.isinf(param.grad).any():
                    print(f"Inf detected in fine model gradient of {name}")
                    return False, train_psnrs, val_psnrs
